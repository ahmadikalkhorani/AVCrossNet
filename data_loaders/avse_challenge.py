...
class Renderer:
    ...
    def render(
        ...
        norm = np.max(np.abs(all_signals)) / 0.9
        
        mix = signal_at_ear / norm
        clean = target_at_ear / norm
        noise = interferer_at_ear / norm
...
class AVSEDataset(Dataset):
    ...
    def __init__(
        ...
        visual_type: str = "embedding_avsr",
        use_all_targets: bool = True,
        num_spks: int = 1,
        ...
        with open(config_dir + f'/avse2_{dataset}.json') as f:
            scenes = json.load(f)
        ...
        self.noise_scenes = {
            "speech": [d for d in scenes if d["interferer"]["type"] == "speech"],
            "noise" : [d for d in scenes if d["interferer"]["type"] == "noise"],
        }
        
        if noise_type != "both":
            if ((use_all_targets) & (dataset == "train") & (dynamic_mix | snr_sch)):
                print("Using the target of both interferers for training")
                self.scenes = scenes
            else: # for valid and test, or when dynamic mixing is off, we only use the specified interferer
                self.scenes = [d for d in scenes if d["interferer"]["type"] == noise_type]
        else: 
            self.scenes = scenes
    ...
    def read(self, wav_path: str,) -> Tuple[torch.Tensor, int]:
        ...
        clean = torch.from_numpy(clean).reshape(1, -1)
        

        return clean, samplerate

    def read_embedding(self, path, t1: float, t2: float, n_frames: int): 
        n1 = int(t1 * self.fps)
        n2 = int(t2 * self.fps) if t2 is not None else None
        path = path.replace(".mp4", ".npy").replace("scenes/", "")
        if self.visual_type.lower() == "embedding_autovsr":
            folder = "/FaceEmbAutoVSR/"
        else: 
            folder = "/FaceEmb/" 
            
        path = "/".join( path.split("/")[:-2] ) + folder + "/".join( path.split("/")[-2:] )
    ...
    def read_face(self, path, t1: float, t2: float, n_frames: int):
        if "embedding" in self.visual_type.lower():
            return self.read_embedding(path, t1, t2, n_frames)
    ...
    def __getitem__(self, idx: Dict[str, int]) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, Any]]:  # type: ignore
        ...
        # if True:
            ...
            target_wav_path = os.path.join(self.wav_dir, scene["dataset"], 'scenes', scene["scene"] + "_target.wav")
            target, sr = self.read(target_wav_path)
            
            
        
            noise_scene = {} # for logging purposes in val/test mode
            if self.snr_sch or self.DM:
                
                if self.snr_sch:
                    noise_scale = self.noise_scale if self.noise_type == "noise" else (-15, +5)
                elif self.noise_type == "both":
                    noise_scale = [-15, 10]
                else: 
                    noise_scale = (-10,+10) if self.noise_type == "noise" else (-15, +5)

                if self.noise_type == "both":
                    # if self.mixed_noise:
                    #     noise_scenes = [
                    #         self.noise_scenes["speech"][randint(g, 0, len(self.noise_scenes["speech"]))],
                    #         self.noise_scenes["noise"][randint(g, 0, len(self.noise_scenes["noise"]))],
                    #     ]
                    #     noise_paths = [os.path.join(self.wav_dir, noise_scene["dataset"], 'scenes', noise_scene["scene"] + "_interferer.wav") for noise_scene in noise_scenes]
                    #     noises = [self.read_noise(noise_path, N = target.shape[-1], g = g) for noise_path in noise_paths]
                    #     noise = torchaudio.functional.add_noise(noises[0], noises[1], snr = torch.tensor([randfloat(g, -50, 50,)]), ) 
                    #     noise_path = noise_paths  
                    # else: 
                        noise_type = "speech" if randfloat(g, 0, 1) < self.speech_noise_probability else "noise"
                        noise_scene = self.noise_scenes[noise_type][randint(g, 0, len(self.noise_scenes[noise_type]))] 
                        noise_path = os.path.join(self.wav_dir, noise_scene["dataset"], 'scenes', noise_scene["scene"] + "_interferer.wav")
                        noise = self.read_noise(noise_path, target.shape[-1], g = g)
                ...
                if noise_type == "speech":
                    ...
                    noise_scale = (-10,+10) if noise_type == "noise" else (-15, +5)

                snr = randfloat(g, *noise_scale)

                
                
                target, mix, noise = self.renderer.render(
                    target = target.reshape(-1).numpy(),
                    interferer = noise.reshape(-1).numpy(),
                    snr_dB = snr, 
                )
                
                if index == 100:
                    print(idx, snr, noise_path,)

            else: 
                noise_scene = scene
                noise_path = os.path.join(self.wav_dir, scene["dataset"], 'scenes', scene["scene"] + "_interferer.wav")
                noise, sr = self.read(noise_path, )
                snr = None

                mix_path = os.path.join(self.wav_dir, scene["dataset"], 'scenes', scene["scene"] + "_mixed.wav")
                mix, sr = self.read(mix_path,)
            ...
            if self.num_spks == 2:
                target = torch.cat([target, noise], dim = 0)
            ...
            if frames:
                if T < frames:
                    mix = torch.cat([mix, torch.zeros(*mix.shape[:-1], frames - T)], dim=-1)
                    target = torch.cat([target, torch.zeros(*target.shape[:-1], frames - T)], dim=-1)
                elif T > frames:
                    start = randint(g, 0, T - frames)
                    start = (start // self.fps) * self.fps
                    mix = mix[:, start:start + frames]
                    target = target[:, :, start:start + frames] 
                end = start + frames
                    
            max_amp = max(torch.max(torch.abs(mix)), torch.max(torch.abs(target)))
            ...
            if self.av:
                sr = self.sample_rate
                path = os.path.join(self.wav_dir, scene["dataset"], 'scenes', scene["scene"] + "_silent.mp4")
                v = self.read_face(path, t1 = start / sr, t2 = end / sr if end is not None else None, n_frames = int(paras["num_samples"]/self.sample_rate * self.fps))
                
                if self.scene_classification:
                    z = torch.tensor([ noise_scene["interferer"]["type"] == "speech" ], dtype= torch.float)
                
                    return x, y, z, v, paras
            ...
            print(f'error in reading {index}: {e}')
...
class SS_SemiOnlineSampler(DistributedSampler[T_co]):
    ...
    def __iter__(self) -> Iterator[T_co]:
        ...
        for i, sidx in enumerate(speech_indices):
            ...
            indices.append({'speech_index': sidx, 'seed': seed, 'dynamic_seed': i + self.epoch, })
...
class AVSEDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        assert noise_type in ["noise", "speech", "both"]
        
        self.noise_type = noise_type

        rank_zero_info(f'dataset: AVSE!, datasets for train/valid/test:  {sample_rate}, time length: {audio_time_len}')
    ...
    def setup(self, stage=None):
        ...
        self.train = AVSEDataset(
            ...
            use_all_targets = self.use_all_targets,
            num_spks = self.num_spks,
            scene_classification = self.scene_classification, 
            mixed_noise= self.mixed_noise,
            batch_size=self.batch_size,
            speech_noise_probability = self.speech_noise_probability,
            
        )
        self.val = AVSEDataset(
            dataset_dir=self.dataset_dir,
            dataset='valid',
        ...
        self.test = AVSEDataset(
            dataset_dir=self.dataset_dir,
            dataset='test',
            audio_time_len=self.audio_time_len[2]  if stage != 'test' else None,
            sample_rate=self.sample_rate,
            noise_type = self.noise_type,
            audio_only=self.ao,
            config_dir = self.config_dir,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--audio_time_len", type=float, default=10.0)
    parser.add_argument("--batch_size", type=list, default=[8,8])
    parser.add_argument("--dynamic_mix", type=bool, default=True)
    parser.add_argument("--noise_type", type=str, default="both")
    parser.add_argument("--visual_type", type=str, default="embedding")
    parser.add_argument("--use_all_targets", type=bool, default=False)
    ...
    x, y, v, p = batch
    # print(f"x shape: {x.shape}, y shape: {y.shape}, v shape: {v.shape}, paras: {p}")
