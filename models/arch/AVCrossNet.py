...
class cross_attention_fusion(nn.TransformerEncoderLayer):
...
class RandomizedPositionalEncoding(nn.Module):
    def __init__(self, F, C, Tmax = 2500):
        """
...
class RandomChunkPositionalEncoding(nn.Module):
    def __init__(self, F, C, Tmax = 7000):
        """
        :param channels: The last dimension of the tensor you want to apply pos emb to.
        """
...
class VisualConv1D(nn.Module):
    def __init__(self, d_visual):
        super(VisualConv1D, self).__init__()
        relu = nn.ReLU()
        norm_1 = nn.BatchNorm1d(d_visual)
        dsconv = nn.Conv1d(d_visual,
                           d_visual,
                           3,
                           stride=1,
                           padding=1,
                           ...
                           bias=False)
        prelu = nn.PReLU()
        norm_2 = nn.BatchNorm1d(d_visual)
        pw_conv = nn.Conv1d(d_visual, d_visual, 1, bias=False)
...
class AVCrossNet(nn.Module):
    ...
    def __init__(
            ...
            positional_encoding_type: str = "lstm", # random or lstm or random_chunk
        ...
        self.encoder_audio = nn.Sequential(
            nn.Conv1d(in_channels=dim_input, out_channels=dim_hidden, kernel_size=encoder_kernel_size, stride=1, padding="same"), 
            # new_norm("LN", dim_hidden, seq_last=True, group_size=None, num_groups=8),
        )
        self.encoder_video = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=dim_hidden, kernel_size=encoder_kernel_size, stride=1, padding="same"),
            # new_norm("LN", dim_hidden, seq_last=True, group_size=None, num_groups=8),
            nn.PReLU()
        )
        ...
        if positional_encoding:
            ...
            if positional_encoding_type == "random":
                self.pe_a = RandomizedPositionalEncoding(C = dim_hidden, F = num_freqs)
                self.pe_v = RandomizedPositionalEncoding(C = dim_hidden, F = num_freqs)
            elif positional_encoding_type == "random_chunk":
                self.pe_a = RandomChunkPositionalEncoding(C = dim_hidden, F = num_freqs)
                self.pe_v = RandomChunkPositionalEncoding(C = dim_hidden, F = num_freqs)
            elif positional_encoding_type == "lstm":
                self.pe_a = LSTMPositionalEncdoing(
                        emb_dim = dim_hidden,
                        emb_ks = 4,
                        emb_hs = 1,
                        n_freqs = num_freqs, 
                ...
                self.pe_v = LSTMPositionalEncdoing(
                        emb_dim = dim_hidden,
                        emb_ks = 4,
                        emb_hs = 1,
                        n_freqs = num_freqs, 
        ...
        for l in range(num_layers):
            ...
            # ll = nn.Linear( 2*dim_hidden, dim_hidden) 
        ...
        self.av_linear_projection = nn.Linear( (1 + dim_output//2)*dim_hidden, dim_hidden) 
        # self.lstm_fusion = nn.LSTM(2*dim_hidden, dim_hidden, batch_first = True)
        # torch.nn.init.xavier_uniform_(self.av_linear_projection.weight)
        ...
        ve_blocks = []
        for _ in range(5):
            ve_blocks += [VisualConv1D(d_visual)]
        ve_blocks += [nn.Conv1d(d_visual, num_freqs, 1)]
        self.visual_conv = nn.Sequential(*ve_blocks)
    
    def forward_video(self, v: Tensor, T: int) -> Tensor:
        # video 
        B = v.shape[0]
        v = rearrange(v, "B Tv Fv -> B Fv Tv")
        v = self.visual_conv(v)
        v = torch.nn.functional.interpolate(v, (T), mode='linear')  # B x Fv x T
        v = v.unsqueeze(-2)  # B x Fv x 1 x T
        v = rearrange(v, "B Fv H T -> (B Fv) H T") # H = 1
        v = self.encoder_video(v) # B 
        v = rearrange(v, "(B F) C T -> B F T C", B = B) # 
        
        # if self.pe_v is not None:
        #     if self.pe_type == "lstm":
        #         v = rearrange(v, "B F T C -> B C T F")
        #         v = self.pe_v(v) 
        #         v = rearrange(v, "B C T F -> B F T C")
        #     else: 
        #         v = self.pe_v(v) 
        return v

    def forward(self, x: Tensor, v:Tensor, return_attn_score: bool = False) -> Tensor:
        # x: [Batch, Freq, Time, Feature]
        B, F, T, H0 = x.shape
        x = x.reshape(B * F, T, H0).permute(0, 2, 1).contiguous() 
        
        x = self.encoder_audio(x).permute(0, 2, 1).contiguous()
        H = x.shape[2]
        ...
        if v.ndim == 3:
            v = v.unsqueeze(1)
        v = torch.cat([self.forward_video(v[:, i], T = T) for i in range(v.shape[1])], dim = -1)
        ...
        x = torch.cat([x, v], dim = -1).contiguous()
        
        
            
        x = self.av_linear_projection(x) 
...
if __name__ == '__main__':
    ...
    # fine unused parameters 
    loss = torch.nn.MSELoss()(y, y)
    loss.backward()
