...
class TrainModule(pl.LightningModule):
    ...
    def __init__(
        ...
        asr_model_name: str = "asr_trlrwlrs2lrs3vox2avsp_base",
    ...
    def forward(self, x: Tensor, v: Tensor = None) -> Tensor:
        ...
        v = v.squeeze(1) if isinstance(v, torch.Tensor) else v
        ...
        if "emb" not in self.visual_type and v is not None:
    ...
    def training_step(self, batch, batch_idx):
        ...
        yr = ys[:, :, self.channels.index(self.ref_channel), :]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        pred = self.forward(x, v)
        yr_hat = pred["preds"]
        loss, perms, yr_hat = self.loss.test(preds=yr_hat, target=yr, reorder=True, reduce_batch=True)
